...
} from "../src/SingleAddressLinearERC20TransferVesting.sol";
...
contract SingleAddressLinearERC20TransferVestingTest is Test {
    ...
    function getVesting(uint96 amount, uint16 duration, uint16 timePassed, uint16 timeAgoStarted, address beneficiary)
        internal
        returns (SingleAddressLinearERC20TransferVesting vesting, uint128 expected)
    {
        vm.assume(duration != 0 && block.timestamp > timeAgoStarted && beneficiary != address(0));
        vesting = new SingleAddressLinearERC20TransferVestingStandalone(
    ...
    function test_linearVesting(
        ...
        (SingleAddressLinearERC20TransferVesting vesting, uint128 expected) =
    ...
    function test_beforeStart(uint96 amount, uint16 duration, uint16 startsIn, address beneficiary) public {
        ...
        SingleAddressLinearERC20TransferVesting vesting = new SingleAddressLinearERC20TransferVestingStandalone(
    ...
    function test_token(IERC20 token, uint128 amount, uint64 start, uint64 duration, address beneficiary) public {
        SingleAddressLinearERC20TransferVesting vesting =
            new SingleAddressLinearERC20TransferVestingStandalone(token, amount, start, duration, beneficiary);
        vm.assertEq(address(vesting.token()), address(token));
    }

    function test_amount(IERC20 token, uint128 amount, uint64 start, uint64 duration, address beneficiary) public {
        SingleAddressLinearERC20TransferVesting vesting =
            new SingleAddressLinearERC20TransferVestingStandalone(token, amount, start, duration, beneficiary);
        vm.assertEq(vesting.amount(), amount);
    }

    function test_start(IERC20 token, uint128 amount, uint64 start, uint64 duration, address beneficiary) public {
        SingleAddressLinearERC20TransferVesting vesting =
            new SingleAddressLinearERC20TransferVestingStandalone(token, amount, start, duration, beneficiary);
        vm.assertEq(vesting.start(), start);
    }

    function test_duration(IERC20 token, uint128 amount, uint64 start, uint64 duration, address beneficiary) public {
        SingleAddressLinearERC20TransferVesting vesting =
            new SingleAddressLinearERC20TransferVestingStandalone(token, amount, start, duration, beneficiary);
        vm.assertEq(vesting.duration(), duration);
    }

    function test_beneficiary(IERC20 token, uint128 amount, uint64 start, uint64 duration, address beneficiary)
        public
    {
        SingleAddressLinearERC20TransferVesting vesting =
            new SingleAddressLinearERC20TransferVestingStandalone(token, amount, start, duration, beneficiary);
